# Delta Spec: `tpv-opcionales` (tpvmod plugin) — tpvmod-opcional-rapido

> **New capability.** Post-archive source of truth:
> `plugins/tpvmod/openspec/specs/tpv-opcionales/spec.md`.
> Plugin-local; `plugins/catalogo_core/` is consumed read-only.
> Traceability: `OD-n` = confirmed decision (`decisions-pending.md` §RESOLVED);
> `F-n` = finding (`research.md` §6).
...
- Research: `plugins/tpvmod/openspec/changes/tpvmod-opcional-rapido/research.md`
- View-layer delta: `plugins/tpvmod/openspec/changes/tpvmod-opcional-rapido/specs/views/spec.md`
- Unchanged: `tpv-flow`, `tpvmod-config`
